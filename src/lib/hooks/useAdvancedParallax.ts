...
export const useAdvancedParallax = ({
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      setOffset({
        x: direction === 'left' ? -currentOffset : direction === 'right' ? currentOffset : 0,
        y: direction === 'up' ? -currentOffset : direction === 'down' ? currentOffset : 0
      });
    };
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      if (rect.top <= stickyDistance) {
        setIsSticky(true);
        const stickyProgress = Math.min(1, (stickyDistance - rect.top) / stickyDistance);
