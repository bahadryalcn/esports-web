...
export default function Footer({ navigation, settings }: FooterProps) {
  ...
  const aboutText =
    ...
    "Türkiye'nin önde gelen e-spor ajansı. Profesyonel oyuncu yönetimi, turnuva organizasyonu ve gaming içerik üretimi.";
  ...
  const footerSections = navigation?.footer?.sections || [
    ...
    },
    {
      ...
      links: [
        {
          href: '/services/esports-management',
          label: 'E-spor Yönetimi',
          external: false,
        },
        {
          href: '/services/tournament-organization',
          label: 'Turnuva Organizasyonu',
          external: false,
        },
        {
          href: '/services/streaming-content',
          label: 'Streaming & İçerik',
          external: false,
        },
        {
  ...
  const getSocialColor = (url: string) => {
    if (url.includes('twitter.com') || url.includes('x.com'))
      return 'hover:bg-blue-500/20 hover:border-blue-500/50';
    ...
    if (url.includes('discord.gg') || url.includes('discord.com'))
      return 'hover:bg-indigo-500/20 hover:border-indigo-500/50';
    if (url.includes('steamcommunity.com') || url.includes('steampowered.com'))
      return 'hover:bg-gray-500/20 hover:border-gray-500/50';
    if (url.includes('tiktok.com'))
      return 'hover:bg-gray-800/20 hover:border-gray-600/50';
    if (url.includes('kick.com'))
      return 'hover:bg-green-500/20 hover:border-green-500/50';
    if (url.includes('facebook.com'))
      return 'hover:bg-blue-600/20 hover:border-blue-600/50';
    if (url.includes('linkedin.com'))
      return 'hover:bg-blue-700/20 hover:border-blue-700/50';
    return 'hover:bg-red-500/20 hover:border-red-500/50';
  };
  ...
  const getSocialFilter = (url: string) => {
    ...
    if (url.includes('steamcommunity.com') || url.includes('steampowered.com'))
      return 'brightness(0) saturate(100%) invert(75%) sepia(0%) saturate(1969%) hue-rotate(344deg) brightness(103%) contrast(101%)';
    if (url.includes('tiktok.com'))
    ...
    if (url.includes('facebook.com'))
      return 'brightness(0) saturate(100%) invert(40%) sepia(96%) saturate(3157%) hue-rotate(200deg) brightness(103%) contrast(104%)';
    if (url.includes('linkedin.com'))
      return 'brightness(0) saturate(100%) invert(30%) sepia(96%) saturate(3157%) hue-rotate(200deg) brightness(103%) contrast(104%)';
  ...
  return (
    <footer className="relative overflow-hidden border-t border-red-500/30 bg-black/90 backdrop-blur-md">
      {/* Background Pattern */}
      ...
      <div className="container relative z-10 mx-auto px-4 py-16">
        ...
        >
          ...
          <motion.div variants={itemVariants} className="space-y-6">
            <Link href="/" className="group flex items-center space-x-3">
              ...
              >
                <div className="relative z-10 flex min-h-[48px] min-w-[48px] items-center justify-center rounded-xl border-2 border-red-400 bg-gradient-to-br from-red-500/60 to-red-700/80 p-4 shadow-xl shadow-red-500/30 backdrop-blur-sm">
                  {siteSettings?.logo?.footer ? (
                    ...
                    <Gamepad2 className="h-12 w-12 text-red-200 transition-colors group-hover:text-red-100" />
                  )}
                </div>
                <div className="absolute inset-0 rounded-xl bg-gradient-to-br from-red-500/20 to-red-700/30 blur-lg transition-all duration-300 group-hover:blur-xl" />
              </motion.div>
              <span className="bg-gradient-to-r from-red-400 to-red-600 bg-clip-text font-gaming text-2xl font-bold text-transparent">
                {siteName}
              </span>
            </Link>
            ...
            <div className="space-y-3">
              <motion.div
                className="group flex items-center space-x-3 text-gray-400 transition-colors hover:text-red-300"
                whileHover={{ x: 5 }}
                transition={{ type: 'spring', stiffness: 300 }}
              >
                <div className="rounded-lg border border-red-500/20 bg-gradient-to-br from-red-500/10 to-red-700/20 p-2 transition-all duration-300 group-hover:border-red-500/40">
          ...
          {footerSections.map((section: FooterSection, index: number) => (
            <motion.div
              ...
              className="space-y-6"
            >
              <h3 className="bg-gradient-to-r from-red-400 to-red-600 bg-clip-text font-gaming text-lg font-bold text-transparent text-white">
                {section.title}
              </h3>
                  ...
                  >
                    {link.external ? (
                      <a
                        ...
                        className="group flex items-center space-x-2 text-sm text-gray-400 transition-colors duration-300 hover:text-red-300"
                      >
                        <span>{link.label}</span>
                        <ExternalLink className="h-3 w-3 opacity-0 transition-all duration-300 group-hover:opacity-100" />
                      </a>
                    ) : (
                      <Link
                        href={link.href}
                        className="text-sm text-gray-400 transition-colors duration-300 hover:text-red-300"
                      >
                        {link.label}
                      </Link>
                    )}
        ...
        <motion.div
          ...
          whileInView="visible"
          viewport={{ once: true }}
          className="mt-12 flex flex-col items-center justify-between border-t border-red-500/20 pt-8 md:flex-row"
        >
          {/* Social Media */}
          <div className="mb-6 flex space-x-3 md:mb-0">
            {socialLinks.map((social, index) => (
              <motion.a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className={`rounded-xl border border-red-500/30 bg-gradient-to-br from-red-500/10 to-red-700/20 p-3 backdrop-blur-sm transition-all duration-300 ${social.color} group`}
          ...
          {/* Copyright */}
          <motion.p
            className="text-center text-sm text-gray-400 md:text-right"
