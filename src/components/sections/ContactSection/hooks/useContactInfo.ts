...
export const useContactInfo = (customContactInfo?: TinaCMSContactInfo[] | ContactInfo[]): ContactInfo[] => {
  ...
  if (customContactInfo && customContactInfo.length > 0) {
    const convertedContactInfo = customContactInfo.map((info: TinaCMSContactInfo | ContactInfo) => {
      let icon: any;
