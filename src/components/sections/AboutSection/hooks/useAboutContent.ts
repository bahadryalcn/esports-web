export const useAboutContent = (customContent?: string) => {
  const defaultContent = {
    ...
    secondary:
      '2020 yılında kurulan ajansımız, kısa sürede e-spor dünyasında saygın bir konum elde etti. Şampiyonluk seviyesindeki performansımız ve müşteri memnuniyetine odaklı yaklaşımımızla sektörde öncü olmaya devam ediyoruz.',
    vision:
      "Vizyonumuz, Türkiye'nin en büyük gaming ajansı olmak ve uluslararası arenada ülkemizi başarıyla temsil etmektir.",
  };
  ...
  if (customContent) {
    ...
    const hasMission =
      customContent.includes('Misyonumuz') || customContent.includes('Mission');
    const hasVision =
      customContent.includes('Vizyonumuz') || customContent.includes('Vision');

    if (hasMission && hasVision) {
      // Content has both mission and vision, extract them
      const missionMatch = customContent.match(
        /(?:Misyonumuz|Mission)[:\s]*([\s\S]*?)(?=\n|Vizyonumuz|Vision|$)/
      );
      const visionMatch = customContent.match(
        /(?:Vizyonumuz|Vision)[:\s]*([\s\S]*?)(?=\n|Misyonumuz|Mission|$)/
      );

      return {
        main: customContent,
        secondary: missionMatch
          ? missionMatch[1].trim()
          : defaultContent.secondary,
